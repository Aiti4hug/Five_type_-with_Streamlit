...
fiv_gray = FiveG().to(device)
fiv_rgb = FiveR().to(device)

fiv_gray.load_state_dict(torch.load('mod/five_gray.pth', map_location=device))
fiv_gray.eval()

fiv_rgb.load_state_dict(torch.load('mod/five_rgb.pth', map_location=device))
fiv_rgb.eval()
